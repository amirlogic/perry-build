...
- fix: `Array.prototype` methods — `reverse()`, `sort()` (no comparator, string-default), `fill(value)`, `concat(other)`, and `Array.from(iter, mapFn)` now work. New runtime functions `js_array_reverse`, `js_array_sort_default`, `js_array_fill`, `js_array_concat_new`; codegen dispatches them in the generic array-method path when the receiver is a known array. `Array.from(iter, mapFn)` desugars at HIR level to `ArrayMap { ArrayFrom(iter), mapFn }`. `js_array_is_array` now returns NaN-boxed TAG_TRUE/TAG_FALSE instead of 1.0/0.0. `matrix[i].length` fixed by adding `Expr::IndexGet` to the dynamic-length fallback list. `test_edge_arrays` passes with zero diff vs Node.
